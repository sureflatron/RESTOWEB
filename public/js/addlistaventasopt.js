    ...
    $.ajax({
        ...
        success: function ($route) {
            ...
            $($route).each(function (key, value) {
                ...
                if (saldo == 0 && value.formaPago == "Credito") {
...
//                    $("#etiqueta").attr("style", "display :block;");
                    ...
                    if (estados == "Guardado" && value.etapa !== "proforma") {
                        ...
                        tabladatos.append("<tr>" +
                                ...
                                "</button>" +
                                "</td><td>" +
                                "<button class='btn btn-danger btn-floating btn-floating' value=" + value.id + " OnClick='Eliminar(this);' title='Eliminar' title='Eliminar'>" +
                                "<i class='material-icons'>delete</i>" +
                                "</button>" +
                                "</td><td>" +
    ...
    if ($("#perfilpuedeGuardar").val() == 1) {
        ...
        $.get(route, function (res) {
            $(res).each(function (key, value) {
                window.location.href = "/Ventaopt/" + value.id;
            });
        });
    } else {
...
function Mostrar($btn) {
    ...
    if ($("#perfilpuedeModificar").val() == 1) {
        var idventa = $btn;
        window.location.href = "/editarventasopt/" + idventa;
    } else {
