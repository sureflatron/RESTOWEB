...
window.onload = function () {
    ...
    cargaralmacen();
    cargarproducto();
    /* var idAlmacen = $("#almacencombo").val();
    ...
    $.ajax({
        ...
        success: function ($route) {
            ...
            var colore = [];
            var cantidad=[];
            var cant,prod,falta,prodid
         $($route).each(function(key, value) {
             prod=value.nombre;   
             prodid=value.idProducto;  
             cant = value.cantidad_optima;
             cantidadoptima.push(cant);
             etiqueta.push(prod)
             producto_id.push(prodid)
             falta=value.n_pedidos;
            if(falta<0 )
            { falta=falta*-1 }                   
            n_pedidos.push(falta)
            cantidad.push(value.inventario_promedio)
              }                       
            );       
    ...
    series: [
	{
        name: 'cantidad_optima',
        data: (function() {
                   var data = [];
                   // debugger
                   cantidadoptima.forEach(function(element) {
        ...
        data:  (function() {
                   ...
                   //alert(stock.pop())
                   // debugger
                   n_pedidos.forEach(function(element) {
                    //   debugger                      
                     data.push([ parseInt(element)]);
                        });                                
                return data;
        ...
        data:  (function() {
                   var data = [];
                   //  debugger
                   producto_id.forEach(function(element) {
                     //  debugger                    
                     data.push([ (element)]);
                        });                                     
                return data;
...
{      debugger
        ...
        // var dia = FechaEntrega.getDate();
         /*var mes = FechaEntrega.getMonth()+1;         
         var anio= FechaEntrega.getFullYear()*/
         //'2017-05-26'
         //var almacen=
           var almacen = $("#almacencombo").val();
           // alert(valor);
           
         var dia = parseInt(FechaEntrega.substr(8,9));
        var mes = parseInt(FechaEntrega.substr(5,6));  
       // var mes2=String(mes) 
       // alert(mes2)
        var anio = parseInt(FechaEntrega.substr(0,4));
    var token = $("#token").val();            
    $.ajax({
        url: '/faltanterafa2/'+mes+'/'+almacen,
        headers: {'X-CSRF-TOKEN': token},
        type: 'GET',
        dataType: 'json',        
        /*data: {
//            anio: anio, 
    ...
    yAxis: {
        min: 0,
        title: {
            text: 'Productos Faltantes OSM SMB'
        },
        stackLabels: {
            enabled: true,
            style: {
...
function cargarproducto(){
    ...
    $('#productoid')
            ...
            .remove()
            .end();
    $('#productoid').material_select();
   
    //var route = "/listaralmacensucursal/" + idpunto;
    var route = "/listarproductosucursal/" + idpunto;
    $.get(route, function (res) {
        $(res).each(function (key, value) {
            $('#productoid').append('<option value=' + value.id + ' >' + value.nombre + '</option>');
            $('#productoid').material_select();
        });
    });
}
...
{  debugger
     ...
     var datosABC = $('#datos');
    $.ajax({
        url: '/ejecutareqq/'+idAlmacen+'/'+idProducto,
        headers: {'X-CSRF-TOKEN': token},
        type: 'GET',
        dataType: 'json',        
        success: function ($route) {
